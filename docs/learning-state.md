...
SecuritiesAccount initializes its cash balances and Available Balance to zero and its status to ACTIVE; it accepts a positive cash receipt and rejects a zero amount.
```
...
```text
Add the next verification test: SecuritiesAccount rejects a negative cash receipt.
```
